...
export const usePagos = () => {
  ...
  const isAllSelected = computed(() => 
    filteredRecibos.value.length > 0 && selectedKeys.value.size === filteredRecibos.value.length
  );

  watch(filteredRecibos, (newRecibos) => {
    selectedKeys.value = new Set(newRecibos.map(r => r.codReciboPr));
  }, { deep: true });
  ...
  const setSelectAll = (select: boolean) => {
    if (select) {
      selectedKeys.value = new Set(filteredRecibos.value.map(r => r.codReciboPr));
    } else {
